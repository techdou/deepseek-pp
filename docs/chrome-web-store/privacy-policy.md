...
DeepSeek++ is a browser extension that enhances the DeepSeek web app with user-controlled memory, skills, prompt presets, MCP tools, inline tool execution, and automation.
...
DeepSeek++ has one purpose: to enhance the DeepSeek web chat experience on `chat.deepseek.com` with memory, skills, presets, tools, and automation that the user controls.
...
- `*://chat.deepseek.com/*`: run on the DeepSeek web app so the extension can inject selected context, detect tool-call markup, render tool results, and support automation inside DeepSeek conversations.
...
DeepSeek++ 是一个浏览器扩展，用于在 DeepSeek 网页版中提供用户可控的长期记忆、技能、提示词预设、MCP 工具、内联工具执行和自动化任务。

本隐私政策说明扩展会处理哪些数据、如何使用这些数据、何时可能传输数据，以及用户可以如何控制自己的数据。

## 1. 单一用途

DeepSeek++ 的单一用途是在 `chat.deepseek.com` 上增强 DeepSeek 网页聊天体验，提供由用户控制的记忆、技能、预设、工具和自动化能力。
...
- `*://chat.deepseek.com/*`：在 DeepSeek 网页版中运行，用于注入用户选择的上下文、识别工具调用标记、展示工具结果，并支持 DeepSeek 对话内的自动化。
